...
export function indxToXy(width: number, height: number): Map<number, [number, number]> {
...
export function leftNeighbors(width: number, height: number): Map<number, number | null> {
...
export function findNeighbors(width: number, height: number): Map<number, (number | null)[]> {
    const toXy = indxToXy(width, height);
...
export function findNeighborsTrim(width: number, height: number): Map<number, number[]> {
    const nbrs = findNeighbors(width, height);
    const map = new Map<number, number[]>();
    for (const [k, v] of nbrs) {
        map.set(k, v.filter((x): x is number => x !== null));
    }
    return map;
}